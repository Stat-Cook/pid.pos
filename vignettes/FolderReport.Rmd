---
title: "PID detection on every file in a folder"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{PID detection on every file in a folder}
...
``` {r, echo=F}
kable(read.csv("Proper Noun Report/Emma.csv", nrows = 5))
```

From these reports - the data controller can inspect the `token` and `sentence` fields and shortlist any `Affected Columns` that need removal.
